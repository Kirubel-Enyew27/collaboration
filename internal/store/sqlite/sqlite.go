...
func (s *SQLiteStore) migrate() error {
	stmts := []string{
		`CREATE TABLE IF NOT EXISTS rooms (name TEXT PRIMARY KEY, created_at
		DATETIME DEFAULT CURRENT_TIMESTAMP)`,
		`CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT,
		room TEXT, type TEXT, payload BLOB, version INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
	}
	for _, q := range stmts {
